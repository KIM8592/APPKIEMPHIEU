...
- Mức H: BẮT BUỘC có (1) Khen + (2) Hướng cải thiện rõ ràng. PHẢI chứa "cần/nên/cố gắng/rèn luyện"
- Mức Đ: BẮT BUỘC có (1) Khen + (2) Hướng cải thiện rõ ràng. PHẢI chứa "cần/nên/cố gắng/rèn luyện"
...
2. TUYỆT ĐỐI KHÔNG nhắc tên học sinh.
3. KHÔNG dùng: "cô", "thầy", "giáo viên".
4. KHÔNG nhắc lại tên các năng lực (Ngôn ngữ, Tính toán, Khoa học, Công nghệ, Tin học, Thẩm mĩ, Thể chất).
5. Diễn đạt cụ thể việc làm của từng năng lực thay vì tên năng lực.
6. Tổng hợp 7 năng lực thành 1 nhận xét liền mạch, tự nhiên.
7. Mỗi học sinh nhận xét khác nhau về cách diễn đạt.
8. Tự động sửa lỗi chính tả, ngữ pháp.
9. Mỗi câu phải tự nhiên như giáo viên thật viết.
...
(KHÔNG giải thích, KHÔNG ký tự dư)`,
  ...
  specificCompetenciesUser: (studentList, aiPrompt, competencyData) => `NỘI DUNG CHỌN:
${aiPrompt || "Không có nội dung"}

DỮ LIỆU NĂNG LỰC (Cấp độ: T=Tốt, Đ=Đạt, H=Hoàn thành, C=Cần):
${competencyData}
...
Trả về theo format: [StudentName]|||[Comment]
Nhận xét tổng hợp 7 năng lực, không lặp lại tên từng năng lực, mô tả cụ thể việc làm.`
};
...
const normalizeYearNameToPair = (name) => {
  ...
  return nums?.length >= 2 ? [parseInt(nums[0], 10), parseInt(nums[1], 10)] : null;
};
...
const normalizeMonthNameToNumber = (name) => {
  ...
  for (const [k, v] of Object.entries(monthMap)) if (s.includes(k)) return v;

  const en = ['january','february','march','april','may','june','july','august','september','october','november','december'];
  return en.findIndex(m => s.includes(m)) + 1 || null;
};

// Tối ưu: Gộp filterComments + beautifyComment + autoFixComment
const processComment = (comment, level) => {
  if (!comment) return comment;
  
  // Loại bỏ tên giáo viên
  comment = comment
    .replace(/(?:co|thầy|giáo\s+viên)\s+(?:muốn|yêu cầu|mong|hy vọng|khuyến khích)\s+em/gi, 'Em hãy')
    .replace(/(?:co|thầy|giáo\s+viên)\s+tin\s+(?:rằng\s+)?em/gi, 'Em có khả năng')
    .replace(/(?:co|thầy|giáo\s+viên)\s+(?:nói|bảo)\s+em/gi, 'Em')
    .replace(/(?:co|thầy|giáo\s+viên)\s+(?:nhận\s+xét|khen)/gi, '')
    .replace(/,\s*(?:co|thầy|giáo\s+viên)(?:\s+[^!.?]*)?(?=[!.?])/gi, '')
    .replace(/\b(?:co|thầy|giáo\s+viên|co\s+giáo|thầy\s+giáo)\b/gi, '')
    .replace(/\s+/g, ' ').trim()
    .replace(/^[,\s]+/, '');
  ...
  const hasDevelopment = /(cần|nên|hãy|tiếp tục|phát huy|duy trì|thử sức|rèn luyện|ôn|tăng|cải thiện|nâng cao|tiến bộ|tốt hơn|phát triển)/i.test(comment);
  const isPraise = /(tốt|vững|hoàn thành|đúng|đạt|khá|nắm vững|chăm|tích cực)/i.test(comment);
  const isBroken = comment.split(" ").length < 5 || /(và|nhưng|song|giúp|làm cho|để)$/i.test(comment);
...
const App = () => {
  ...
  const showToast = useCallback((message, type = 'info', icon = 'ℹ️', duration = 4000) => {
  ...
  const updateDraft = useCallback((studentId, field, value) => {
  ...
  const handleUpdateStatus = useCallback(async (studentId, status) => {
  ...
  const handleDrop = useCallback(async (e, targetStudentId) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  }, [draftData, systemMode, viewMode, selectedYearId, selectedSubId, selectedMonthId, selectedClassId, selectedCriteriaId, subjects, user, showToast]);

  // ===== AI GENERATION - TỐI ƯU =====
  ...
  return false;
    ...
    try {
      let studentListText = "";
let competencyDataText = ""; // THÊM

// ===== KIỂM TRA NẾU LÀ SMAS SPECIFIC =====
const isSmaSSpecific = systemMode === 'smas' && viewMode === 'specific';
...
allTargets.forEach((stu, idx) => {
  ...
  if (isSmaSSpecific) {
    // ===== CHO SMAS SPECIFIC =====
    const compDetails = SPECIFIC_COMPETENCIES.map(c => {
      const lv = draft[`level_${c.id}`] !== undefined ? draft[`level_${c.id}`] : (d[`level_${c.id}`] || "");
      return lv ? `${c.name}:${lv}` : null;
    }).filter(Boolean).join(", ");
    info = compDetails || "N/A";
    competencyDataText += `${idx + 1}. ${stu.name}: ${compDetails}\n`;
...
// ===== XÁC ĐỊNH PROMPT VÀ INSTRUCTION =====
let userInstruction = "";
let systemPrompt = "";

if (isSmaSSpecific) {
  userInstruction = AI_PROMPTS.specificCompetenciesUser(studentListText, aiPrompt, competencyDataText);
  systemPrompt = AI_PROMPTS.specificCompetencies;
} else {
  userInstruction = AI_PROMPTS.user(studentListText, aiPrompt);
    ...
    generationConfig: { 
      ...
      lines.forEach(line => {
        ...
        if (parts.length >= 2) {
          ...
          if (student && comment && comment.length > 10) {
...
// Chỉ dùng level thường nếu không phải specific competencies
let level = draft.level !== undefined ? draft.level : (d.level || "");

// Nếu là SMAS specific: xác định level từ 7 năng lực
if (systemMode === 'smas' && viewMode === 'specific') {
  const levels = SPECIFIC_COMPETENCIES.map(c => 
    draft[`level_${c.id}`] !== undefined ? draft[`level_${c.id}`] : (d[`level_${c.id}`] || "")
  ).filter(Boolean);
  
  // Priority: C > Đ > H > T
  if (levels.includes('C')) {
    level = 'C';
  } else if (levels.includes('Đ')) {
    level = 'Đ';
  } else if (levels.includes('H')) {
    level = 'H';
  } else if (levels.includes('T')) {
    level = 'T';
  }
}
...
comment = processComment(comment, level);
      ...
      setDraftData(prev => {
        ...
        for (const sId of Object.keys(updates)) {
          updated[sId] = { ...(prev[sId] || {}), comment: updates[sId] };
        }
        return updated;
      });

      setIsGenerating(false);
      showToast(`Thành công: ${successCount}/${allTargets.length} học sinh`, 'success', '✅', 3000);
  ...
  }, [isAuthValid, apiKey, students, studentData, draftData, viewMode, systemMode, selectedSubId, selectedCriteriaId, subjects, aiPrompt, showToast]);

  const handleSendZalo = useCallback((studentId) => {
  ...
  }, [studentData, draftData, students, viewMode, systemMode, selectedSubId, selectedCriteriaId, subjects, months, selectedMonthId, showToast]);
  ...
  }, [students, studentData, draftData, classes, months, subjects, viewMode, systemMode, selectedClassId, selectedMonthId, selectedSubId, selectedCriteriaId, showToast]);
                ...
                <div className="flex-1 w-full text-left">
                  ...
                  <div className="relative">
                    ...
                    <input className="w-full bg-slate-50 border-2 border-slate-300 rounded-xl pl-12 pr-4 py-4 text-sm font-semibold outline-none focus:border-indigo-500 transition-all" 
                      placeholder="VD: hãy random nội dung sau: cộng có nhớ, giải toán, hình học. khoảng 20 từ " 
                      value={aiPrompt} 
...
>👥</button>
  ...
  {/* Nhóm bên trái: Mức đạt & Ghi chú */}
  <div className="flex gap-3">
    <button onClick={() => setShowLevel(!showLevel)} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-[11px] font-black uppercase transition-all border ${showLevel ? 'bg-slate-800 text-white border-slate-800' : 'bg-white text-slate-400 border-slate-200'} hover:shadow-sm`}>
      {showLevel ? '👁️' : '🚫'} Mức đạt
    </button>
    <button onClick={() => setShowNote(!showNote)} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-[11px] font-black uppercase transition-all border ${showNote ? 'bg-slate-800 text-white border-slate-800' : 'bg-white text-slate-400 border-slate-200'} hover:shadow-sm`}>
      {showNote ? '👁️' : '🚫'} Ghi chú
    </button>
  </div>
